...
class SimulatedMarketDataTests(unittest.TestCase):
    """
    This app does not fetch or redistribute real exchange market data (see
    PSX's market data licensing notice). These tests assert that quotes and
    historical candles are always simulated, for any ticker/market.
    """

    def setUp(self):
        QUOTE_CACHE.clear()

    def test_pk_quote_is_always_simulated(self):
        quote = get_latest_quote("MARI", market="PK")
        self.assertIsNotNone(quote)
        self.assertFalse(quote.get("is_live"))
        self.assertEqual(quote.get("source"), "simulated")

    def test_unknown_ticker_and_market_still_returns_simulated_quote(self):
        quote = get_latest_quote("ZZZZ", market="US")
        self.assertIsNotNone(quote)
        self.assertFalse(quote.get("is_live"))
        self.assertEqual(quote.get("source"), "simulated")
        self.assertGreater(quote.get("price", 0), 0)

    def test_historical_data_is_generated_for_any_ticker(self):
        df = generate_historical_data("ZZZZ", days=30, market="US")
